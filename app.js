(() => {
  ...
  function init() {
    if (!QUIZ || !Array.isArray(QUIZ.sets)) {
      document.body.innerHTML =
        '<main style="padding:24px;font-family:Arial,sans-serif"><h1>Không tải được dữ liệu câu hỏi</h1><p>Hãy kiểm tra file data.js có nằm cùng thư mục với index.html không.</p></main>';
      return;
    }
  ...
  function bindGlobalEvents() {
    els.clearAllBtn.addEventListener("click", () => {
      if (
        !confirm(
          "Bạn có chắc muốn xóa toàn bộ tiến trình làm bài đã lưu không?",
        )
      )
        return;
    ...
    els.retryBtn.addEventListener("click", () => {
      ...
      if (
        !confirm(
          `Làm lại Đề ${setNumber}? Đáp án sẽ được đảo ngẫu nhiên lại từ đầu.`,
        )
      )
        return;
    ...
    els.reviewBtn.addEventListener("click", () => {
      ...
      els.reviewBtn.textContent = els.reviewList.classList.contains(
        "review-list--show",
      )
        ? "Ẩn phần xem lại"
        : "Xem lại bài";
  ...
  function renderMenu() {
    ...
    QUIZ.sets.forEach((set) => {
      ...
      const statusText = !saved
        ? "Chưa làm"
        : saved.finished
          ? `Đã làm: ${stats.correct}/${set.questions.length}`
          : `Đang làm: ${stats.answered}/${set.questions.length}`;
      ...
      button.addEventListener("click", () => {
        ...
        if (action === "new") {
          ...
          if (
            saved &&
            !confirm(
              "Đề này đã có tiến trình/kết quả cũ. Làm lại sẽ đảo đáp án và xóa tiến trình cũ. Tiếp tục?",
            )
          )
            return;
          startNewAttempt(setNumber);
        ...
        ${answer.options
          .map((option, index) => {
            const shouldRevealCorrect =
              answer.selectedIndex !== null || state.finished;
            const isWrongSelection =
              answer.selectedIndex === index && !option.isCorrect;
            const classes = [
              "option-btn",
  ...
  function goToQuestion(index) {
    ...
    renderPalette();
    const cardTop =
      els.questionCard.getBoundingClientRect().top + window.scrollY - 18;
  ...
  function finishExam(reason) {
    ...
    renderResult();
    showToast(
      reason === "timeout" ? "Hết giờ. Hệ thống đã tự nộp bài." : "Đã nộp bài.",
    );
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  function renderResult() {
    ...
    els.scoreRing.style.setProperty(
      "--p",
      `${(stats.correct / set.questions.length) * 360}deg`,
    );
